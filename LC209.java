package LeetCode;

public class LC209 {
    public int minSubArrayLen(int s, int[] nums) {
        if (nums.length == 0) return 0;
        // nums[start...end]为滑动窗口
        int start = 0, end = -1;
        int sum = 0;

        int minLength = nums.length + 1;
        // nums[start...end] 为和大于s的数组，如果小于s则end++,如果大于s则start--
        // minLength记录最小长度，数组的遍历长度为end-start+1
        while (start < nums.length) {
            if (end + 1 < nums.length && sum < s) {
                sum += nums[++end];
            } else {
                sum -= nums[start++];
            }
            if (sum>=s){
                minLength = Math.min(end - start + 1, minLength);
            }
        }
        if (minLength == nums.length + 1)
            return 0;
        return minLength;
    }

    /***
     * C(nlogN)级别算法
     * @param s
     * @param nums
     * @return
     */
    public int minSubArrayLenLiuyubo(int s, int[] nums) {

        if(s <= 0 || nums == null)
            throw new IllegalArgumentException("Illigal Arguments");

        int[] sums = new int[nums.length + 1];
        sums[0] = 0;
        for(int i = 1 ; i <= nums.length ; i ++)
            sums[i] = sums[i-1] + nums[i-1];

        int res = nums.length + 1;
        for(int l = 0 ; l < nums.length; l ++){
            // Unfortunately, there's no lowerBound method in Java，
            // We need to implement our own lowerBound :(
            int r = lowerBound(sums, sums[l] + s);
            if(r != sums.length)
                res = Math.min(res, r - l);
        }

        return res == nums.length + 1 ? 0 : res;
    }

    // Find the smallest number index which is larger or equal to target
    // in a sorted array nums
    // If there's no such a number, in which all number in nums are smaller than target
    // return nums.length
    private int lowerBound(int[] nums, int target){

        if(nums == null /*|| !isSorted(nums)*/)
            throw new IllegalArgumentException("Illegal argument nums in lowerBound.");

        int l = 0, r = nums.length;
        while(l != r){
            int mid = l + (r - l) / 2;
            if(nums[mid] >= target)
                r = mid;
            else
                l = mid + 1;
        }

        return l;
    }

    private boolean isSorted(int[] nums){
        for(int i = 1 ; i < nums.length ; i ++)
            if(nums[i] < nums[i-1])
                return false;
        return true;
    }

    public int minSubArrayLenBamzhyWrong(int s, int[] nums) {
        if (nums.length == 0) return 0;
        int minLength = nums.length + 1;
        int start = 0;
        int end = -1;
        int result = 0;

        //这里错了，关注点是start是否到了最后一个
        while (end < nums.length - 1) {
            if (result < s) {
                result += nums[++end];
            } else {
                // 这里也有问题
                minLength = Math.min(end - start + 1, minLength);
                result -= nums[start++];
            }
        }
        return minLength;
    }

    public static void main(String[] args) {
        int[] nums = {2, 3, 1, 2, 4, 3};
        new LC209().minSubArrayLen(7, nums);
    }
}
